...
const program = new Command('nova-commandkit');

program
    .name('nova-commandkit')
    .description('Beginner friendly command & event handler for Discord.js (Node.js 24+ ready).')
    .version('1.0.0');

program
    .command('dev')
    .description('Start your bot in development mode.')
    .option('-c, --config <path>', 'Path to your nova-commandkit config file.', './commandkit.js')
    .action(() => {
...
program
    ...
    .option('-c, --config <path>', 'Path to your nova-commandkit config file.', './commandkit.json')
    .action(() => {
        const options = program.opts();
